...
export interface AdvancedCanvasPluginSettingsValues {
  ...
  customEdgeConnectionPositions: boolean // 新增：允许自定义边缘连接位置
...
export const DEFAULT_SETTINGS_VALUES: AdvancedCanvasPluginSettingsValues = {
  ...
  customEdgeConnectionPositions: true,
...
export const SETTINGS = {
  ...
  floatingEdgeFeatureEnabled: {
    label: 'Floating Edge',
    description: 'Automatically change the side of the edge connection when moving nodes.',
    infoSection: 'floating-edges-automatic-edge-side',
    disableToggle: false,
    children: {
      allowFloatingEdgeCreation: {
        label: 'Allow Floating Edge Creation',
        description: 'Allow creating floating edges by dropping the edge connection on the center of the node.',
        type: 'boolean'
      },
      newEdgeFromSideFloating: {
        label: 'New Edge From Side Floating',
        description: 'Make the from side of new edges floating by default.',
        type: 'boolean'
      },
      customEdgeConnectionPositions: {
        label: 'Custom Edge Connection Positions',
        description: 'Allow edges to connect at any position along the node edge, not just at the center.',
        type: 'boolean'
      }
    }
  },
